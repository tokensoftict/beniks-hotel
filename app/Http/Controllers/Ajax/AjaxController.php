...
class AjaxController extends Controller
{
    ...
    public function findclothtype(Request $request)
    {
        $result = [];
        $query = ($request->get('query') ? $request->get('query') : $request->get('searchTerm'));
        if(empty($query)) {
        ...
        $query =  explode(' ', $query);
        $cloths = Cloth::with(['cloth_service_mappers.cloth', 'cloth_service_mappers.laundry_service'])->where(function($sub) use ($request, $query){
            foreach ($query as $char) {
                $sub->where('cloth_name', 'LIKE', "%{$char}%");
            }
        })->get();
