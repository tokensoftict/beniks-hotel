...
class ExpensesController extends Controller
{
    ...
    public function create(){
        ...
        $data['depts'] = Warehousestore::all()->pluck('name');
    ...
    public function expenses_report_by_type(Request $request){
        ...
        return view('expenses.list_by_type',$data);
    }
    ...
    public function expenses_report_by_department(Request $request){
        if($request->get('from') && $request->get('to')){
            ...
            $dpt_key = Warehousestore::all()->pluck('name');
            $data['from'] = date('Y-m-01');
            $data['to'] = date('Y-m-t');
            $data['department'] = Warehousestore::all()->pluck('name')[0];
        }
