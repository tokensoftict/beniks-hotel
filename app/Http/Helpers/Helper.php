...
function getActiveStore($force = false, $warehouse_id = NULL){

    if($warehouse_id !== NULL)
    {
        return Warehousestore::find($warehouse_id);
    }

    if(request()->has('global_filter_store')){
        return Warehousestore::find(request()->get('global_filter_store'));
    }

    $user = auth()->user();
    if($user->warehousestore_id != NULL)
    {
        return Warehousestore::find($user->warehousestore_id);
    }
    if($force == true){
